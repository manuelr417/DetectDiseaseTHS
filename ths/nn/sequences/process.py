from ths.nn.sequences.tweets import TweetSentiment2LSTM, TweetSentiment3LSTM, TweetSentiment2LSTM2Dense, TweetSentiment2LSTM2DenseSM, TweetSentimentGRUSM, TweetSentimentCNN, TweetSentiment2DCNN
...
class ProcessTweetsWord2VecOnePassSM:
    ...
    def process(self, json_filename, h5_filename, plot=False, epochs = 100, vect_dimensions = 100):
        ...
        print("Train data padded")
        trim_size = 40
        T = TrimSentences(trim_size)
        X_train_pad = T.trim_list(X_train_pad)
        #convert to numPY arrays
        X_train = np.array(X_train_pad)
        Y_train = np.array(Y_train)
        Y_train = to_categorical(Y_train, num_classes=3)
        ...
        kernel_regularizer = l2(0.001)
        #kernel_regularizer = None
        NN.build(first_layer_units = 100, second_layer_units=100, relu_dense_layer=100, dense_layer_units=3, first_layer_dropout=0.5, second_layer_dropout=0.5, l2 = kernel_regularizer)
        print("model built")
        NN.summary()
        sgd = SGD(lr=0.01, momentum=0.009, decay=0.001, nesterov=True)
        rmsprop = RMSprop(decay=0.002)
        adam = Adam(lr=0.1, decay=0.05)
        NN.compile(optimizer=rmsprop, loss="categorical_crossentropy", metrics=['accuracy'])
        print("model compiled")
        print("Begin training")
        ...
        print(X_Predict)
        X_Predict_Final = P.pad_list(X_Predict_Idx)
        X_Predict_Final = T.trim_list(X_Predict_Final)
        #X_Predict = [X_Predict]
        X_Predict_Final = np.array(X_Predict_Final)
        print("Predict: ", NN.predict(X_Predict_Final))
        print("Storing model and weights")
...
class ProcessTweetsWord2VecOnePass2DCNN:
    ...
    def process(self, json_filename, h5_filename, plot=False, epochs = 100, vect_dimensions = 100):
        ...
        Y_train = to_categorical(Y_train, num_classes=3)
        print("Train data convert to numpy arrays")
        NN = TweetSentiment2DCNN(max_len, G)
        #print("Build GRU")
        #NN = TweetSentimentGRUSM(max_len, G)
        ...
        kernel_regularizer = l2(0.001)
        #kernel_regularizer = None
        NN.build(filters=3, kernel_size=5, first_dropout=0.05, second_dropout=0.1, padding='valid')
        print("model built")
        NN.summary()
        sgd = SGD(lr=0.03, momentum=0.009, decay=0.001, nesterov=True)
        rmsprop = RMSprop(decay=0.003)
